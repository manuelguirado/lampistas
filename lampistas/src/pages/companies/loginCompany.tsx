...
export default function LoginCompany() {
  ...
  function handleSubmitCompanyCode(data: LoginSchema) {
    api
      .post("/company/companyLoginCode", data)
      .then((response) => {
  ...
  function handleSubmitEmail(data: LoginSchema) {
    api
      .post("/company/companyLogin", data)
      .then((response) => {
        const responseData = response.data;
        if (responseData.token) {
          toast.success(t("loginExitoso"));
    ...
    <div className="w-full min-h-screen flex flex-col bg-gradient-to-br from-amber-50 to-orange-100 items-center pt-20 md:pt-24 px-4 pb-8">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 border border-amber-100">
          {/* Email/Password Form */}
          <form
            onSubmit={handleSubmitEmailForm(handleSubmitEmail)}
            className="space-y-6"
          >
            {/* Email Input */}
          ...
          {/* Código de empresa */}
          <form
            onSubmit={handleSubmitCodeForm(handleSubmitCompanyCode)}
            className="space-y-4"
          >
